# Rax — Command Line User Interface for Claude Code
...
- **Skills marketplace** — install plugins from Anthropic's GitHub repos without leaving Rax.
...
The fastest way to get Rax running as a regular Mac app. This installs dependencies, voice support (Whisper), builds the app, copies it to `/Applications`, and launches it.
...
After the initial install, just open **Rax** from your Applications folder or Spotlight.
...
| `./commands/stop.command` | Stop all Rax processes |
...
> **No API keys or `.env` file required.** Rax uses your existing Claude Code CLI authentication (Pro/Team/Enterprise subscription).
...
├── preload/                # Secure IPC bridge (window.rax API)
...
Rax operates almost entirely offline. The only outbound network calls are:
...
- **Requires Claude Code CLI** — Rax is a UI layer, not a standalone AI client. You need an authenticated `claude` CLI.
