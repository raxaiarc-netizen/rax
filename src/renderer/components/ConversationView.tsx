...
import {
  ...
  SpinnerGap, ArrowCounterClockwise, Square, Waveform, Camera, CursorClick, Monitor,
...
export function ConversationView() {
  ...
  // Orb tab uses its own controls (right-click orb to reset, Esc to dismiss).
  const showInterrupt = !tab.isOrbTab && isRunning && tab.messages.some((m) => m.role === 'user')

  if (tab.messages.length === 0) {
    return tab.isOrbTab ? <OrbEmptyState /> : <EmptyState />
  }
  ...
  return (
    ...
    >
      {/* Scrollable messages area — mask fades the bottom edge to transparent so text softly disappears (no darkening, no shadow) */}
      <div
        ref={scrollRef}
        ...
        style={{
          ...
          paddingBottom: 28,
          maskImage: 'linear-gradient(to bottom, black 0%, black calc(100% - 36px), transparent 100%)',
          WebkitMaskImage: 'linear-gradient(to bottom, black 0%, black calc(100% - 36px), transparent 100%)',
        }}
      ...
      >
        ...
        <div className="space-y-1 relative">
          {grouped.map((item, idx) => {
            ...
            const orbVariant: 'orb' | 'normal' = tab.isOrbTab ? 'orb' : 'normal'

            switch (item.kind) {
              case 'user':
                return <UserMessage key={item.message.id} message={item.message} skipMotion={isHistorical} variant={orbVariant} />
              case 'assistant':
              ...
              case 'system':
                return <SystemMessage key={item.message.id} message={item.message} skipMotion={isHistorical} variant={orbVariant} />
              default:
                return null
            }
          })}
      ...
      {/* Activity row — overlaps bottom of scroll area; status + interrupt only (text fade is handled by the scroll mask above) */}
      <div
      ...
      >
        ...
        <div className="flex items-center gap-1.5 text-[11px] min-w-0 relative">
          {isRunning && (
            <span className="flex items-center gap-1.5">
              <span className="flex gap-[3px]">
                <span className="w-[4px] h-[4px] rounded-full animate-bounce-dot" style={{ background: tab.isOrbTab ? colors.orbAccent : colors.statusRunning, animationDelay: '0ms' }} />
                <span className="w-[4px] h-[4px] rounded-full animate-bounce-dot" style={{ background: tab.isOrbTab ? colors.orbAccent : colors.statusRunning, animationDelay: '150ms' }} />
                <span className="w-[4px] h-[4px] rounded-full animate-bounce-dot" style={{ background: tab.isOrbTab ? colors.orbAccent : colors.statusRunning, animationDelay: '300ms' }} />
              </span>
...
// ─── Empty state for the pinned voice-orb tab ───
// Different from the regular EmptyState: no folder picker (orb has its
// own working dir), no welcome text, just an inline tab-picker so the
// user can point the orb at any existing chat tab's history.

function OrbEmptyState() {
  const colors = useColors()
  return (
    <div
      className="flex flex-col items-center justify-center px-6 py-6 gap-1.5 text-center"
      style={{ minHeight: 110 }}
    >
      <div
        className="flex items-center justify-center w-9 h-9 rounded-full"
        style={{
          background: colors.orbAccentLight,
          border: `1px solid ${colors.orbAccentBorder}`,
          color: colors.orbAccent,
          boxShadow: `0 0 18px ${colors.orbAccentGlow}`,
        }}
      >
        <Waveform size={18} weight="bold" />
      </div>
      <div className="text-[13px] font-medium" style={{ color: colors.orbAccent }}>
        Voice
      </div>
      <div className="text-[11px]" style={{ color: colors.textTertiary, maxWidth: 320 }}>
        Summon the orb with <strong style={{ color: colors.textSecondary }}>⌘⇧O</strong> or hold
        <strong style={{ color: colors.textSecondary }}> ⌥R</strong> for push‑to‑talk. The full
...
function UserMessage({ message, skipMotion, variant = 'normal' }: { message: Message; skipMotion?: boolean; variant?: 'normal' | 'orb' }) {
  const colors = useColors()
  const orb = variant === 'orb'
  const showChip = orb && !!message.hasAutoScreenshot
  const bubble = (
    <div
      className="text-[13px] leading-[1.5] px-3 py-1.5 max-w-[85%]"
      style={{
        background: orb ? colors.orbBubble : colors.userBubble,
        color: orb ? colors.orbBubbleText : colors.userBubbleText,
        border: `1px solid ${orb ? colors.orbBubbleBorder : colors.userBubbleBorder}`,
  ...
  const chip = showChip ? (
    <div
      className="flex items-center gap-[5px] text-[10px] leading-none px-2 py-1 mt-1 rounded-[10px]"
      style={{
        background: colors.orbBubble,
        color: colors.orbBubbleText,
        border: `1px solid ${colors.orbBubbleBorder}`,
        opacity: 0.85,
      }}
  ...
  if (skipMotion) {
    return <div className="flex justify-end py-1.5">{wrapper}</div>
  }
...
function ImageCard({ src, alt, colors }: { src?: string; alt?: string; colors: ReturnType<typeof useColors> }) {
  ...
  const open = () => { if (src) window.rax.openExternal(String(src)) }
  ...
  const markdownComponents = useMemo(() => ({
    ...
    a: ({ href, children }: any) => (
      <button
        ...
        onClick={() => {
          if (href) window.rax.openExternal(String(href))
        }}
      >
        {children}
      </button>
...
function getToolDescriptionFromParsed(name: string, parsed: Record<string, unknown>): string {
  ...
  // MCP screen tools — match the suffix so the description is server-agnostic.
  if (name.endsWith('__rax_screenshot')) {
    const display = parsed.display
    return typeof display === 'number' ? `Capture screen (display ${display})` : 'Capture screen'
  }
  if (name.endsWith('__rax_control_screen')) {
    const action = s(parsed.action)
    switch (action) {
      case 'click': return `Click at ${parsed.x},${parsed.y}`
      case 'double_click': return `Double-click at ${parsed.x},${parsed.y}`
      case 'type': {
        const text = s(parsed.text)
        return `Type "${text.length > 40 ? text.substring(0, 37) + '...' : text}"`
      }
      case 'key': {
        const mods = Array.isArray(parsed.modifiers) ? (parsed.modifiers as string[]).join('+') : ''
        return `Press ${mods ? mods + '+' : ''}${s(parsed.key)}`
      }
      case 'scroll': return `Scroll dx=${parsed.dx ?? 0} dy=${parsed.dy ?? 0}`
      case 'cursor_position': return 'Read cursor position'
      default: return action ? `Screen ${action}` : 'Control screen'
    }
  }
                ...
                <div key={tool.id} className="relative">
                  ...
                  <div className="min-w-0">
                    ...
                    {!isRunning && parsedInput && (() => {
                      const monoFont = "'Poppins', -apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Segoe UI', system-ui, sans-serif"
...
function SystemMessage({ message, skipMotion, variant = 'normal' }: { message: Message; skipMotion?: boolean; variant?: 'normal' | 'orb' }) {
  const isError = message.content.startsWith('Error:') || message.content.includes('unexpectedly')
  const colors = useColors()
  const isOrbDivider = variant === 'orb' && /^—\s*new voice conversation\s*—$/i.test(message.content)

  if (isOrbDivider) {
    const divider = (
      <div className="flex items-center gap-2 py-2">
        <span className="flex-1 h-px" style={{ background: colors.orbAccentBorder }} />
        <span className="text-[10px] tracking-wide uppercase" style={{ color: colors.orbAccent, letterSpacing: '0.08em' }}>
          new voice conversation
        </span>
        <span className="flex-1 h-px" style={{ background: colors.orbAccentBorder }} />
      </div>
    )
    return skipMotion ? <div>{divider}</div> : (
...
function ToolIcon({ name, size = 12 }: { name: string; size?: number }) {
  ...
  // MCP tools come through as `mcp__<server>__<tool>` — match the suffix so
  // the icon doesn't depend on which MCP server registered them.
  if (name.endsWith('__rax_screenshot')) {
    return (
