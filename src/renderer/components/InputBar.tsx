...
// The pinned voice-orb tab is read-only — the orb itself is the input.
// Swap the composer for a "Summon orb" pill + a "Reset history" button
// that wipes the voice transcript and resets the orb's claude session.
function OrbInputPlaceholder() {
  const colors = useColors()
  const messageCount = useSessionStore((s) => s.tabs.find((t) => t.isOrbTab)?.messages.length ?? 0)
  const canReset = messageCount > 0

  const handleReset = useCallback(() => {
    if (!canReset) return
    // Optimistic local wipe; main's ORB_RESET_BROADCAST will re-apply.
    useSessionStore.getState().applyOrbReset()
    try { void window.rax.resetOrb() } catch {}
  }, [canReset])

  return (
    <div className="w-full flex items-center gap-2 px-1.5" style={{ minHeight: 50 }}>
      <button
        type="button"
        onClick={() => { void window.rax.showOrb() }}
        className="flex items-center gap-2 px-3 py-1.5 rounded-full transition-transform hover:scale-[1.02]"
        title="Summon the voice orb (⌘⇧O)"
        style={{
      ...
      <button
        ...
        title={canReset ? 'Wipe the voice transcript and start a fresh orb session' : 'Voice transcript is already empty'}
        style={{
          color: canReset ? colors.orbAccent : colors.textTertiary,
          background: 'transparent',
          border: `1px dashed ${canReset ? colors.orbAccentBorder : colors.toolBorder}`,
          fontSize: 11,
...
export function InputBar() {
  ...
  const isAwaitingPermission = (tab?.permissionQueue?.length ?? 0) > 0
  const hasContent = input.trim().length > 0 || (tab?.attachments?.length ?? 0) > 0
  // Lock send while a permission card is pending — the user must answer it
  // before queuing more work, just like terminal CLI's modal y/n prompt.
  const canSend = !!tab && !isConnecting && !isAwaitingPermission && hasContent
  ...
  useEffect(() => {
    const unsub = window.rax.onWindowShown(() => {
      textareaRef.current?.focus()
    })
    return unsub
  }, [])
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === 'Escape' && !showSlashMenu) { window.rax.hideWindow() }
  }
  ...
  const startRecording = useCallback(async () => {
    ...
    recorder.onstop = async () => {
      ...
      try {
        ...
        const result = await window.rax.transcribeAudio(wavBase64)
    ...
    recorder.onerror = () => { stream.getTracks().forEach((t) => t.stop()); setAudioStream(null); setVoiceError('Recording failed.'); setVoiceState('idle') }
  ...
  // Pinned voice tab is read-only — swap the composer for the orb
  // placeholder + context picker so the pill geometry stays identical.
  if (tab?.isOrbTab) {
    return <OrbInputPlaceholder />
  }

  return (
    <div ref={wrapperRef} data-rax-ui className="flex flex-col w-full relative">
      {/* Slash command menu */}
      <AnimatePresence>
        {showSlashMenu && (
          <SlashCommandMenu
          ...
          <div className="w-full">
            {voiceState === 'recording' ? (
              <div className="flex items-center w-full" style={{ minHeight: 50 }}>
                <div className="flex-1 min-w-0" style={{ paddingTop: 9, paddingBottom: 9, paddingRight: 4 }}>
                  <LiveWaveform
                    active
                    stream={audioStream}
                    height={32}
