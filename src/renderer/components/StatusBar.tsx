...
import { Terminal, CaretDown, Check, FolderOpen, Plus, X, ShieldCheck, Warning } from '@phosphor-icons/react'
...
function ModelPicker() {
  ...
  const activeLabel = (() => {
    ...
    const fallback = AVAILABLE_MODELS.find((m) => m.id === DEFAULT_MODEL_ID)
    return fallback?.label || AVAILABLE_MODELS[0].label
  })()
...
function PermissionModePicker() {
  ...
  const triggerLabel = isBypass ? 'Bypass' : isAuto ? 'Auto' : 'Ask'
  const triggerColor = isBypass ? '#ef4444' : colors.textTertiary
  ...
  return (
    <>
      <button
        ...
        title={isBypass ? 'Permission mode: Bypass — all permission checks are skipped' : 'Permission mode (global)'}
      >
        {isBypass
        ...
        >
          <div className="py-1">
            <button
              ...
              className="w-full flex items-center justify-between px-3 py-1.5 text-[11px] transition-colors"
              style={{
                color: isAsk ? colors.textPrimary : colors.textSecondary,
                fontWeight: isAsk ? 600 : 400,
              }}
            >
              ...
              {isAsk && <Check size={12} style={{ color: colors.accent }} />}
            </button>
            ...
            <button
              ...
              className="w-full flex items-center justify-between px-3 py-1.5 text-[11px] transition-colors"
              style={{
                color: isBypass ? '#ef4444' : colors.textSecondary,
                fontWeight: isBypass ? 600 : 400,
              }}
              title="Skip all permission checks — equivalent to claude --dangerously-skip-permissions"
            >
              <span className="flex items-center gap-1.5">
                <Warning size={12} weight="fill" />
                Bypass
              </span>
              {isBypass && <Check size={12} style={{ color: '#ef4444' }} />}
            </button>
          </div>
        </motion.div>,
        popoverLayer,
...
export function StatusBar() {
  ...
  const handleOpenInTerminal = () => {
    window.rax.openInTerminal(tab.claudeSessionId, tab.workingDirectory)
  }
