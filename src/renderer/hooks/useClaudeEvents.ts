...
export function useClaudeEvents() {
  ...
  useEffect(() => {
    ...
    const unsubEvent = window.rax.onEvent((tabId, event) => {
    ...
    const unsubStatus = window.rax.onTabStatusChange((tabId, newStatus, oldStatus) => {
      handleStatusChange(tabId, newStatus, oldStatus)
    })

    const unsubError = window.rax.onError((tabId, error) => {
      handleError(tabId, error)
    })

    const unsubSkill = window.rax.onSkillStatus((status) => {
      if (status.state === 'failed') {
        console.warn(`[RAX] Skill install failed: ${status.name} — ${status.error}`)
      }
    })

    const unsubCodeMode = window.rax.onCodeModeStatus((state) => {
      setCodeModeState(state)
    })

    // Pull current state once at mount in case main is already running.
    window.rax.getCodeModeStatus().then(setCodeModeState).catch(() => {})
  ...
  }, [handleNormalizedEvent, handleStatusChange, handleError, setCodeModeState])

  // Note: window.rax.start() is called via sessionStore.initStaticInfo() in App.tsx.
