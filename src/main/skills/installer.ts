 ...
 *
 * Respects user-managed skills: if a skill dir exists without .rax-version,
...
const VERSION_FILE = '.rax-version'
...
function log(msg: string): void {
  ...
  try { appendFileSync(join(homedir(), '.rax-debug.log'), line) } catch {}
}
...
async function installGithubSkill(
  ...
  try {
    ...
    // Atomic swap: remove old (if RAX-managed), rename tmp into place
    if (existsSync(targetDir)) {
      const existing = readVersionFile(targetDir)
      if (existing?.installedBy === 'rax') {
...
async function installSkill(
  ...
  if (existsSync(targetDir)) {
    ...
    if (!meta) {
      // Dir exists but no .rax-version — user-managed, don't touch
      log(`Skipping ${entry.name}: user-managed (no ${VERSION_FILE})`)
      onStatus({ name: entry.name, state: 'skipped', reason: 'user-managed' })
      return
    }

    if (meta.version === entry.version && meta.installedBy === 'rax') {
