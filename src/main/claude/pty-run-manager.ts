 ...
 *
 * This module is behind the `RAX_INTERACTIVE_PERMISSIONS_PTY` feature flag.
 *
 * Known limitations:
...
function isUiChrome(line: string): boolean {
  ...
  // Status bar fragments: "Opus 4.7 · Claude Max" etc.
...
export class PtyRunManager extends EventEmitter {
  ...
  startRun(requestId: string, options: RunOptions): PtyRunHandle {
    ...
    log(`Starting PTY run ${requestId} [${instance.mode}]: ${command} ${spawnArgs.join(' ')}`)
