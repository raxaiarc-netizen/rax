...
Rax is a **macOS-only Electron overlay** that wraps the Claude Code CLI (`claude -p --output-format stream-json`) in a floating pill UI. It is NOT a web app, NOT a VS Code extension, and does NOT call the Anthropic API directly — it spawns CLI subprocesses.
...
| Debug logging | `RAX_DEBUG=1 npm run dev` (writes to `~/.rax-debug.log`) |
...
| **Preload** | `src/preload/` | Typed IPC bridge (`window.rax` API). Security boundary. |
...
```
InputBar.tsx → window.rax.prompt(tabId, requestId, opts)
...
4. Call from renderer via `window.rax.*`
