...
If you discover a security vulnerability in RAX, please report it responsibly:
...
RAX runs entirely on your local machine. Key security properties:
...
- **Preload isolation** — the renderer has no direct access to Node.js APIs; all IPC goes through a typed `window.rax` bridge.
...
No other network connections are made by RAX itself. The `claude` CLI may make its own connections as part of normal operation.
